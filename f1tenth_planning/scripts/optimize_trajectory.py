...
def load_vehicle_profile(profile_path):
    """Load and validate the vehicle values mirrored by racecar.ini."""
    with open(profile_path, 'r') as f:
        profile = yaml.safe_load(f) or {}

    vehicle = profile.get('vehicle', {})
    geometry = profile.get('geometry', {})
    tire = profile.get('tire', {})
    steering = profile.get('steering', {})
    required = {
        'vehicle.mass_kg': vehicle.get('mass_kg'),
        'vehicle.model_wheelbase_m': vehicle.get('model_wheelbase_m'),
        'geometry.max_steering_rad': geometry.get('max_steering_rad'),
    ...
    if missing:
        raise ValueError(
            f'Vehicle profile {profile_path} is missing: {", ".join(missing)}'
        )

    model_wheelbase = float(vehicle['model_wheelbase_m'])
    max_steering = float(geometry['max_steering_rad'])
    expected_curvlim = math.tan(max_steering) / model_wheelbase
    expected_radius = 1.0 / expected_curvlim
    return {
        'name': str(profile.get('name', 'vehicle')),
        'mass_kg': float(vehicle['mass_kg']),
        'weight_n': float(vehicle.get('weight_n', vehicle['mass_kg'] * 9.81)),
    ...
    for kappa_bound in kappa_candidates:
        for width_opt in width_candidates:
            ...
            try:
                ...
                # A narrow or self-near projected path can make TPH's
                # periodic spline matrix ill-conditioned.  That candidate is
                # not usable, but it must not abort the complete mintime run.
                print(
                    "  Mincurv pre-pass spline check failed: "
                    f"kappa_bound={kappa_bound:.3f}, "
                    f"width_opt={width_opt:.3f}: "
                    f"{type(exc).__name__}: {exc}"
                )
...
def main():
    ...
    vehicle_profile_path = os.path.join(
        workspace, 'f1tenth_planning', 'config', 'autodrive_sim_vehicle.yaml'
    )
    ...
    try:
        vehicle_profile = load_vehicle_profile(vehicle_profile_path)
    ...
    if args.strict_curvlim and (curvlim is None or curvlim <= 0.0):
        ...
        sys.exit(1)
    if curvlim is not None and abs(curvlim - vehicle_profile['expected_curvlim_m_inv']) > 0.005:
        print(
            "ERROR: racecar.ini and vehicle profile disagree on curvature limit: "
            f"racecar.ini={curvlim:.6f}, "
            f"profile={vehicle_profile['expected_curvlim_m_inv']:.6f} 1/m"
        )
        sys.exit(1)
    if curvlim is None:
