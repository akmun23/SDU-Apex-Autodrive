...
TEST_F(FollowTheGapTest, WallMarginDoesNotInflateEmergencyDistance) {
    config_.wall_margin = 0.15;
    config_.emergency_brake_distance = 0.18;
    config_.footprint_clearance = 0.0;
    ftg_->setConfig(config_);

    auto ranges = createCorridorScan(NUM_POINTS);
    const size_t quarter = NUM_POINTS / 4;
    for (size_t i = 0; i < quarter; ++i) {
        ranges[i] = 0.24f;
    }
    for (size_t i = 3 * quarter; i < NUM_POINTS; ++i) {
        ranges[i] = 0.24f;
    }

    auto output = ftg_->compute(ranges, ANGLE_MIN, ANGLE_MAX, ANGLE_INC);
    EXPECT_FALSE(output.emergency_stop);
    EXPECT_NEAR(output.closest_point_dist, 0.24, 0.001);
}

TEST_F(FollowTheGapTest, FootprintGuardAccountsForForwardLidarOffset) {
    config_.car_length = 0.500;
    config_.car_width = 0.270;
    config_.rear_overhang = 0.080;
    config_.lidar_offset_x = 0.2733;
    config_.emergency_brake_distance = 0.18;
    config_.footprint_clearance = 0.08;
    ftg_->setConfig(config_);

    auto ranges = createOpenScan(NUM_POINTS);
    ranges[NUM_POINTS / 2] = 0.21f;
...
TEST_F(FollowTheGapTest, FootprintGuardAllowsConfiguredForwardClearance) {
    config_.car_length = 0.500;
    config_.car_width = 0.270;
    config_.rear_overhang = 0.080;
    config_.lidar_offset_x = 0.2733;
...
TEST_F(FollowTheGapTest, SideMarginExpandsDisparityProtection) {
    auto ranges = createOpenScan(NUM_POINTS);
    for (size_t i = 0; i < NUM_POINTS / 2; ++i) {
        ranges[i] = 0.5f;
    }

    config_.wall_margin = 0.0;
    config_.side_safety_margin = 0.0;
    config_.max_disparity_extension_angle = constants::PI / 4.0;
    ftg_->setConfig(config_);
    auto body_only = ftg_->compute(
        ranges, ANGLE_MIN, ANGLE_MAX, ANGLE_INC);
    const auto body_blocked = std::count(
        body_only.processed_scan.disparity_blocked.begin(),
        body_only.processed_scan.disparity_blocked.end(),
        true);

    ftg_->reset();
    config_.side_safety_margin = 0.10;
    ftg_->setConfig(config_);
    auto guarded = ftg_->compute(
        ranges, ANGLE_MIN, ANGLE_MAX, ANGLE_INC);
    const auto guarded_blocked = std::count(
        guarded.processed_scan.disparity_blocked.begin(),
        guarded.processed_scan.disparity_blocked.end(),
        true);

    EXPECT_GT(guarded_blocked, body_blocked);
}
