...
- The physical Unity anchors are now aligned across the offline/native plant
  defaults: Rigidbody mass `3.47 kg`, projected yaw inertia
  `0.0276985662 kg m^2`, contact wheelbase `0.33000004 m`, steering geometry
  wheelbase `0.324 m`, and contact radius `0.059 m`. The old controller radius
  `0.0325 m` remains documented as skid-steer-only.
- Regime cross-score:
...
1. Collect repeated E5 throttle sequences and explicitly score the four-wheel
  continuous model at arbitrary integration steps.
2. Complete E4 excitation coverage and perform blind body force/moment
  inversion using the known `m` and `I_z`.
3. Extend the regime result with speed/slip-conditioned scheduling or a
   causal drive/load state before reducing the plant for MPC.
